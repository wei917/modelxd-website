...
export default function XEvalPage() {
  ...
  const BENCH_LABEL: Record<string, string> = { gdpval: 'GDPval', 'terminal-bench-2-1': 'Terminal-Bench 2.1', 'harvey-lab': 'Harvey LAB', 'text-rendering': 'Text Rendering', 'sotopia-hard': 'Social (SOTOPIA)' }
  ...
  const BENCH_SLUG: Record<string, string> = { gdpval: 'gdpval', 'terminal-bench-2-1': 'tb', 'harvey-lab': 'lab', 'text-rendering': 'text', 'sotopia-hard': 'social' }
...
function TBSection({ runs, label }: { runs: RunRow[]; label: string }) {
  ...
  return (
    <>
      <p style={{ fontSize: 14.5, color: 'var(--muted2)', lineHeight: 1.7, maxWidth: 760, margin: '0 0 22px' }}>
        {(label === 'Harvey LAB' ? t('xeval.lead.lab') : label === 'Text Rendering' ? t('xeval.lead.textrender') : label === 'Social (SOTOPIA)' ? t('xeval.lead.social') : t('xeval.lead.tb'))
          .replace('{set}', label)
