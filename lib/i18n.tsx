...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.lab.method.body': { en: 'Each entry runs {n} {set} tasks in Harvey’s open-source harness: a real client-matter file set, six workspace tools, no internet. Every task ships a human-written rubric, and a task scores only if EVERY criterion passes — Harvey’s rule, because a memo that catches 95% of the issues and misses one material one is not 95% useful. Two independent judges (GPT-5.6 Luna and Qwen3.8 Max, neither sharing a family with either entry) grade every criterion separately at temperature 0; the headline is the share of judges that all-passed, and Coverage is the share of individual criteria satisfied. Tasks are drawn proportionally across practice areas, and only tasks both entries completed are listed. Cost is not shown: this harness reports no cached-token split, and we would rather omit a number than publish one we cannot measure.', 'zh-Hant': '每個條目在 Harvey 開源框架中執行 {n} 個 {set} 任務：真實客戶案件檔案、六項工作區工具、無網路。每個任務附有人工撰寫的評分標準，且必須「全部」項目通過才算通過——這是 Harvey 的規則：一份抓到 95% 問題卻漏掉一項關鍵問題的備忘錄，並非 95% 有用。兩位獨立評審（GPT-5.6 Luna 與 Qwen3.8 Max，皆與參賽模型不同家族）在溫度 0 下逐項評分；標題數字為全部通過的評審比例，達成率則為個別項目的通過比例。任務按各專業領域比例抽樣，且僅列出兩個條目都完成的任務。不顯示成本：此框架未回報快取 token 拆分，與其發布無法測量的數字，不如省略。', 'zh-Hans': '每个条目在 Harvey 开源框架中执行 {n} 个 {set} 任务：真实客户案件档案、六项工作区工具、无网络。每个任务附有人工撰写的评分标准，且必须「全部」项目通过才算通过——这是 Harvey 的规则：一份抓到 95% 问题却漏掉一项关键问题的备忘录，并非 95% 有用。两位独立评审（GPT-5.6 Luna 与 Qwen3.8 Max，皆与参赛模型不同家族）在温度 0 下逐项评分；标题数字为全部通过的评审比例，达成率则为个别项目的通过比例。任务按各专业领域比例抽样，且仅列出两个条目都完成的任务。不显示成本：此框架未报告缓存 token 拆分，与其发布无法测量的数字，不如省略。', ja: '各エントリはHarveyのオープンソースハーネスで{n}件の{set}タスクを実行します：実際の案件ファイル、6つのワークスペースツール、インターネットなし。各タスクには人手作成のルーブリックが付属し、全項目を満たした場合のみ合格——一つでも重大な見落としがあれば95%有用ではない、というHarveyのルールです。2名の独立審査（GPT-5.6 LunaとQwen3.8 Max、いずれも参加モデルと別系統）が温度0で各項目を個別に採点。見出しは全項目合格とした審査の割合、達成率は個々の項目の充足割合です。タスクは分野ごとに比例抽出し、両エントリが完了したタスクのみ掲載。コストは非表示：このハーネスはキャッシュトークンの内訳を報告せず、測定できない数値を出すより省略します。', ko: '각 엔트리는 Harvey의 오픈소스 하니스에서 {n}개의 {set} 과제를 수행합니다: 실제 사건 파일, 6개 작업 도구, 인터넷 차단. 각 과제에는 사람이 작성한 루브릭이 있으며 모든 기준을 충족해야만 통과입니다 — 95%를 잡아도 중대한 하나를 놓치면 95% 유용한 것이 아니라는 Harvey의 규칙입니다. 두 독립 심사(GPT-5.6 Luna, Qwen3.8 Max — 둘 다 참가 모델과 다른 계열)가 온도 0에서 기준별로 채점하며, 헤드라인은 전 기준 통과한 심사 비율, 충족률은 개별 기준 충족 비율입니다. 과제는 분야별 비례 추출이며 두 엔트리가 모두 완료한 과제만 게시합니다. 비용은 표시하지 않습니다: 이 하니스는 캐시 토큰 내역을 보고하지 않으며, 측정할 수 없는 수치를 게시하느니 생략합니다.' },
