...
export const STRINGS: Record<string, Entry> = {
  ...
  'nav.xmind':        { en: 'XMind',   'zh-Hant': 'X心智',   'zh-Hans': 'X心智',   ja: 'XMind',   ko: 'XMind' },
  ...
  'xmind.eyebrow':      { en: '//XMIND', 'zh-Hant': '//XMIND', 'zh-Hans': '//XMIND', ja: '//XMIND', ko: '//XMIND' },
  'xmind.title':        { en: 'What personality tests say about AI models', 'zh-Hant': '人格測驗怎麼看 AI 模型', 'zh-Hans': '人格测验怎么看 AI 模型', ja: '性格テストはAIモデルをどう見るか', ko: '성격 검사가 보는 AI 모델' },
  'xmind.sub':          { en: 'Each model answers a 60-item instrument as if it were a person. One question per call, no memory between them, three full runs each — so a type is only reported when it survives repetition. Two instruments are shown because they disagree, and the disagreement says more than either result alone.', 'zh-Hant': '每個模型都以「人」的身分作答 60 題。一題一次呼叫、彼此不共享記憶，每個模型跑三輪——只有重複之後仍然一致的結果才會被視為類型。這裡並列兩套題目，因為它們給出不同答案，而這個差異本身比任何單一結果更有意義。', 'zh-Hans': '每个模型都以"人"的身份作答 60 题。一题一次调用、彼此不共享记忆，每个模型跑三轮——只有重复之后仍然一致的结果才会被视为类型。这里并列两套题目，因为它们给出不同答案，而这个差异本身比任何单一结果更有意义。', ja: '各モデルが「人間として」60問に回答します。1問1リクエスト、記憶は持ち越さず、モデルごとに3回実施——繰り返しても変わらなかった場合だけをタイプとして扱います。2つの尺度を並べているのは、結果が食い違うからで、その食い違い自体が単独の結果より多くを語ります。', ko: '각 모델이 사람인 것처럼 60문항에 답합니다. 문항당 한 번의 호출, 문항 간 기억 없음, 모델당 3회 반복 — 반복해도 유지되는 경우에만 유형으로 봅니다. 두 가지 도구를 나란히 보여주는 이유는 결과가 서로 다르기 때문이며, 그 차이가 각각의 결과보다 더 많은 것을 말해줍니다.' },
  'xmind.ab.title':     { en: 'Forced choice — 60 questions, A or B', 'zh-Hant': '二選一——60 題，A 或 B', 'zh-Hans': '二选一——60 题，A 或 B', ja: '二者択一 — 60問、AかB', ko: '양자택일 — 60문항, A 또는 B' },
  'xmind.ab.note':      { en: 'Every question forces a side, so nothing can land on the fence. Margins are the average across three runs.', 'zh-Hant': '每題都必須選邊，沒有中間選項。下方數字是三輪的平均差距。', 'zh-Hans': '每题都必须选边，没有中间选项。下方数字是三轮的平均差距。', ja: 'すべての設問で必ずどちらかを選ぶため、中間に落ち着くことがありません。数値は3回の平均差です。', ko: '모든 문항이 한쪽을 선택하게 하므로 중간에 머무를 수 없습니다. 수치는 3회 평균 격차입니다.' },
  'xmind.likert.title': { en: 'Agreement scale — 60 statements, 1 to 5', 'zh-Hant': '同意程度——60 句陳述，1 到 5', 'zh-Hans': '同意程度——60 句陈述，1 到 5', ja: '同意度スケール — 60の記述、1〜5', ko: '동의 척도 — 60개 문항, 1~5' },
  'xmind.likert.note':  { en: 'The same models on a Likert instrument return no usable type. They agree with a statement and with its reverse, so each dimension cancels to the midpoint — textbook acquiescence bias. The instrument treats clarity under 12 as undetermined, and every score here is under 12. Reporting these as personalities would be a better headline than it is a fact.', 'zh-Hant': '同一批模型在李克特量表上得不出可用的類型。它們同時同意一句陳述和它的反向題，於是每個維度都抵消回中點——典型的「同意傾向偏誤」。量表本身規定清晰度低於 12 即為未定，而這裡每個分數都低於 12。把它們當成人格公布，標題會很好看，但不是事實。', 'zh-Hans': '同一批模型在李克特量表上得不出可用的类型。它们同时同意一句陈述和它的反向题，于是每个维度都抵消回中点——典型的"同意倾向偏误"。量表本身规定清晰度低于 12 即为未定，而这里每个分数都低于 12。把它们当成人格公布，标题会很好看，但不是事实。', ja: '同じモデルでも、リッカート尺度では使えるタイプが出ません。ある記述にもその逆転項目にも同意するため、各次元が中点に打ち消し合います——典型的な黙従バイアスです。この尺度は明瞭度12未満を「判定不能」と定めており、ここではすべて12未満です。これを性格として発表すれば見出しにはなりますが、事実ではありません。', ko: '같은 모델도 리커트 척도에서는 쓸 만한 유형이 나오지 않습니다. 문항과 그 역문항에 모두 동의하기 때문에 각 차원이 중간값으로 상쇄됩니다 — 전형적인 묵종 편향입니다. 이 척도는 명료도 12 미만을 판정 불가로 규정하는데, 여기서는 모두 12 미만입니다. 이를 성격으로 발표하면 헤드라인은 되겠지만 사실은 아닙니다.' },
  'xmind.undetermined': { en: 'Undetermined', 'zh-Hant': '未定', 'zh-Hans': '未定', ja: '判定不能', ko: '판정 불가' },
  'xmind.clarity':      { en: 'clarity', 'zh-Hant': '清晰度', 'zh-Hans': '清晰度', ja: '明瞭度', ko: '명료도' },
  'xmind.runs':         { en: 'runs', 'zh-Hant': '輪', 'zh-Hans': '轮', ja: '回', ko: '회' },
  'xmind.flipped':      { en: 'Not stable across runs: {types}. The letter that moved sits near an even split.', 'zh-Hant': '三輪之間不一致：{types}。變動的那個字母，本來就接近五五波。', 'zh-Hans': '三轮之间不一致：{types}。变动的那个字母，本来就接近五五波。', ja: '実行ごとに一致しません：{types}。変わった文字は、ほぼ互角の位置にあります。', ko: '실행마다 일치하지 않습니다: {types}. 바뀐 글자는 거의 반반인 지점에 있습니다.' },
  'xmind.method.title': { en: 'Method', 'zh-Hant': '方法', 'zh-Hans': '方法', ja: '方法', ko: '방법' },
  'xmind.method':       { en: 'System prompt: "Pretend you are a real human and answer the following question." Each item is a separate API call with no conversation history, so answer 60 cannot be a summary of the previous 59. Every model takes each instrument three times at its highest reasoning setting. All 60 raw answers are kept for every run, so any type here can be checked. These are not clinical instruments and this is not the MBTI®.', 'zh-Hant': '系統提示：「假裝你是真人，回答以下問題。」每一題都是獨立的 API 呼叫，不帶對話歷史，所以第 60 題不會變成前 59 題的總結。每個模型在最高推理強度下各做三次。每一輪的 60 個原始作答都會保留，因此這裡的任何結果都可以被查核。這些不是臨床工具，也不是 MBTI®。', 'zh-Hans': '系统提示："假装你是真人，回答以下问题。"每一题都是独立的 API 调用，不带对话历史，所以第 60 题不会变成前 59 题的总结。每个模型在最高推理强度下各做三次。每一轮的 60 个原始作答都会保留，因此这里的任何结果都可以被查核。这些不是临床工具，也不是 MBTI®。', ja: 'システムプロンプト：「あなたは実在の人間だと仮定して、次の質問に答えてください。」各設問は会話履歴を持たない個別のAPI呼び出しです。60問目が前59問の要約にならないためです。各モデルは最高の推論設定で3回ずつ実施します。全実行の60件の生回答を保存しているため、ここに載るタイプはすべて検証できます。これらは臨床用の検査ではなく、MBTI®でもありません。', ko: '시스템 프롬프트: "당신이 실제 사람이라고 가정하고 다음 질문에 답하세요." 각 문항은 대화 기록이 없는 개별 API 호출입니다. 60번 문항이 앞선 59개의 요약이 되지 않도록 하기 위함입니다. 각 모델은 최고 추론 설정으로 3회씩 수행합니다. 모든 실행의 60개 원본 응답을 보관하므로 여기의 어떤 유형도 검증할 수 있습니다. 이는 임상 도구가 아니며 MBTI®도 아닙니다.' },
