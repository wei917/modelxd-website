...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.lead.lab': { en: 'ModelXD runs {set} — long-horizon legal work over real client-matter files — in Harvey’s own open-source harness. {top} fully passes {solved} of {n} tasks.', 'zh-Hant': 'ModelXD 以 Harvey 官方開源框架執行 {set}——在真實客戶案件檔案上的長流程法律工作。{top} 於 {n} 題中完全通過 {solved} 題。', 'zh-Hans': 'ModelXD 以 Harvey 官方开源框架执行 {set}——在真实客户案件档案上的长流程法律工作。{top} 于 {n} 题中完全通过 {solved} 题。', ja: 'ModelXDはHarvey公式のオープンソースハーネスで{set}を実行します——実際の案件ファイルを扱う長期法務業務です。{top}は{n}件中{solved}件を完全通過。', ko: 'ModelXD는 Harvey 공식 오픈소스 하니스로 {set}을 실행합니다 — 실제 사건 파일을 다루는 장기 법무 업무입니다. {top}은 {n}개 중 {solved}개를 완전 통과했습니다.' },
