...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.lead.gdpval': { en: 'ModelXD runs OpenAI’s GDPval gold tasks with pairwise quality ratings under its own XEval harness. {top} scores {rating}, against {anchor} for the human professional who was paid to do the same work.', 'zh-Hant': 'ModelXD 以自建的 XEval 框架執行 OpenAI 的 GDPval 黃金任務，並以成對品質評比計分。{top} 得 {rating} 分，同一份工作的付費人類專業者為 {anchor} 分。', 'zh-Hans': 'ModelXD 以自建的 XEval 框架执行 OpenAI 的 GDPval 黄金任务，并以成对质量评比计分。{top} 得 {rating} 分，同一份工作的付费人类专业者为 {anchor} 分。', ja: 'ModelXDは自社のXEvalハーネスでOpenAIのGDPvalゴールドタスクを実行し、ペア比較で品質を採点します。{top}は{rating}、同じ仕事を有償で行った人間の専門家は{anchor}です。', ko: 'ModelXD는 자체 XEval 하니스로 OpenAI의 GDPval 골드 과제를 실행하고 쌍대 비교로 품질을 채점합니다. {top}은 {rating}점, 같은 일을 수행한 유급 인간 전문가는 {anchor}점입니다.' },
