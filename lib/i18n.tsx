...
export const STRINGS: Record<string, Entry> = {
  ...
  'la.intro':            { en: 'Hi — I am the ModelXD guide. ModelXD helps you find the AI model that gives the best result for your money: run the same prompt through several models, vote before the price is revealed, then see the leaderboard those blind votes build. Ask me anything — where a feature lives, which model is cheapest, or how to make a video.', 'zh-Hant': '嗨，我是 ModelXD 導覽。ModelXD 幫你找出最划算的 AI 模型：同一個提示詞跑過多個模型、在看到價格之前先投票，再看看這些盲測投票堆出來的排行榜。有什麼都可以問我——功能在哪裡、哪個模型最便宜，或是怎麼做影片。', 'zh-Hans': '嗨，我是 ModelXD 导览。ModelXD 帮你找出最划算的 AI 模型：同一个提示词跑过多个模型、在看到价格之前先投票，再看看这些盲测投票堆出来的排行榜。有什么都可以问我——功能在哪里、哪个模型最便宜，或是怎么做视频。', ja: 'こんにちは、ModelXD のガイドです。ModelXD は、費用に対して最良の結果を出す AI モデルを見つけるお手伝いをします。同じプロンプトを複数のモデルで実行し、価格を見る前に投票し、そのブラインド投票から作られるランキングを確認できます。機能の場所、一番安いモデル、動画の作り方など、何でも聞いてください。', ko: '안녕하세요, ModelXD 가이드입니다. ModelXD는 지불한 만큼 가장 좋은 결과를 주는 AI 모델을 찾도록 도와줍니다. 같은 프롬프트를 여러 모델에 돌리고, 가격을 보기 전에 투표한 뒤, 그 블라인드 투표로 만들어진 랭킹을 확인하세요. 기능이 어디 있는지, 어떤 모델이 가장 저렴한지, 영상은 어떻게 만드는지 무엇이든 물어보세요.' },
