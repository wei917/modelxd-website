...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.method.body': { en: 'Each entry is a (model, reasoning-effort) pair run on {tasks} GDPval gold tasks in the open-source Stirrup harness (Docker sandbox, cached web environment: identical query → identical results). Deliverables are compared pairwise, anonymized and order-randomized, by {judge}; entries submitting no deliverables forfeit by rule. Ratings are Bradley-Terry fits on the same scale as XBoard’s XDRating. Costs are measured from actual token usage at list prices. Ratings on this page are not comparable to GDPval-AA’s — different judges, pool, and anchor.', 'zh-Hant': '每個條目為（模型 × 推理強度）組合，在開源 Stirrup 框架中執行 {tasks} 個 GDPval 黃金任務（Docker 沙盒、快取網路環境：相同查詢 → 相同結果）。交付物經匿名化、隨機排序後由 {judge} 成對評比；未提交交付物者依規則判負。評分採 Bradley-Terry 擬合，與 XBoard 的 XDRating 同一尺度。成本按實際 token 用量以標價計算。本頁評分與 GDPval-AA 不可直接比較——評審、對手池與錨點均不同。', 'zh-Hans': '每个条目为（模型 × 推理强度）组合，在开源 Stirrup 框架中执行 {tasks} 个 GDPval 黄金任务（Docker 沙盒、缓存网络环境：相同查询 → 相同结果）。交付物经匿名化、随机排序后由 {judge} 成对评比；未提交交付物者按规则判负。评分采用 Bradley-Terry 拟合，与 XBoard 的 XDRating 同一尺度。成本按实际 token 用量以标价计算。本页评分与 GDPval-AA 不可直接比较——评审、对手池与锚点均不同。', ja: '各エントリは（モデル × 推論エフォート）の組で、オープンソースのStirrupハーネス上で{tasks}件のGDPvalゴールドタスクを実行（Dockerサンドボックス、キャッシュされたウェブ環境：同一クエリ → 同一結果）。成果物は匿名化・順序ランダム化のうえ {judge} がペア比較し、成果物未提出はルールにより敗北。レーティングはBradley-Terry適合で、XBoardのXDRatingと同一スケール。コストは実トークン使用量×定価で計測。本ページのレーティングはGDPval-AAとは比較できません（審査・プール・アンカーが異なります）。', ko: '각 엔트리는 (모델 × 추론 강도) 조합으로, 오픈소스 Stirrup 하니스에서 {tasks}개의 GDPval 골드 과제를 실행합니다(Docker 샌드박스, 캐시된 웹 환경: 동일 쿼리 → 동일 결과). 결과물은 익명화·순서 무작위화 후 {judge}가 쌍대 비교하며, 결과물 미제출 시 규칙상 패배 처리됩니다. 레이팅은 Bradley-Terry 적합으로 XBoard의 XDRating과 동일한 스케일입니다. 비용은 실제 토큰 사용량 × 정가로 측정됩니다. 이 페이지의 레이팅은 GDPval-AA와 직접 비교할 수 없습니다(심사·풀·앵커가 다릅니다).' },
