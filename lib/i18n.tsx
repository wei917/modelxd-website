...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.method.body': { en: 'Each entry is a (model, reasoning-effort) pair run on {tasks} GDPval gold tasks in the open-source Stirrup harness (Docker sandbox, cached web environment: identical query → identical results). Tasks are balanced across the nine GDPval sectors; entries may have run different subsets, so the games column shows each entry’s coverage. Deliverables are compared pairwise, anonymized and order-randomized, by {judge}; entries submitting no deliverables forfeit by rule. Ratings are Bradley-Terry fits on the same scale as XBoard’s XDRating. Costs are measured from actual token usage at list prices. The judges’ own entries compete in the pool, but no judge ever rates a pair that includes its own model — those pairs are decided by the other judges. Every comparison is anonymized and order-randomized. The ModelXD Router row is ModelXD’s own service, measured: for every task in the library it serves the entry that measured best on that task, so its games and cost are those served runs’ real verdicts and real cost — a measured selection over completed runs, while every other entry ran blind. Ratings on this page are not comparable to GDPval-AA’s — different judges, pool, and anchor.', 'zh-Hant': '每個條目為（模型 × 推理強度）組合，在開源 Stirrup 框架中執行 {tasks} 個 GDPval 黃金任務（Docker 沙盒、快取網路環境：相同查詢 → 相同結果）。任務平均分佈於九個 GDPval 產業；各條目完成的任務數量不一，對局數欄位即顯示各條目的覆蓋範圍。交付物經匿名化、隨機排序後由 {judge} 成對評比；未提交交付物者依規則判負。評分採 Bradley-Terry 擬合，與 XBoard 的 XDRating 同一尺度。成本按實際 token 用量以標價計算。評審模型自身的條目亦參與競賽，但任何評審絕不評比含有自家模型的配對——該配對由其他評審裁定。所有評比皆經匿名化與隨機排序。ModelXD Router 列是 ModelXD 自身服務的實測結果：對題庫中的每個任務，它提供該任務實測最佳的條目，因此其對局與成本即為所提供執行的真實評比與真實花費——是基於已完成測量的選擇；其他條目均為盲測執行。本頁評分與 GDPval-AA 不可直接比較——評審、對手池與錨點均不同。', 'zh-Hans': '每个条目为（模型 × 推理强度）组合，在开源 Stirrup 框架中执行 {tasks} 个 GDPval 黄金任务（Docker 沙盒、缓存网络环境：相同查询 → 相同结果）。任务均衡分布于九个 GDPval 行业；各条目完成的任务数量不一，对局数列即显示各条目的覆盖范围。交付物经匿名化、随机排序后由 {judge} 成对评比；未提交交付物者按规则判负。评分采用 Bradley-Terry 拟合，与 XBoard 的 XDRating 同一尺度。成本按实际 token 用量以标价计算。评审模型自身的条目亦参与竞赛，但任何评审绝不评比含有自家模型的配对——该配对由其他评审裁定。所有评比皆经匿名化与随机排序。ModelXD Router 行是 ModelXD 自身服务的实测结果：对题库中的每个任务，它提供该任务实测最佳的条目，因此其对局与成本即为所提供执行的真实评比与真实花费——是基于已完成测量的选择；其他条目均为盲测执行。本页评分与 GDPval-AA 不可直接比较——评审、对手池与锚点均不同。', ja: '各エントリは（モデル × 推論エフォート）の組で、オープンソースのStirrupハーネス上で{tasks}件のGDPvalゴールドタスクを実行（Dockerサンドボックス、キャッシュされたウェブ環境：同一クエリ → 同一結果）。タスクはGDPvalの9セクターに均等配分。エントリごとに実行済みタスク数は異なる場合があり、対戦数の列が各エントリのカバー範囲を示します。成果物は匿名化・順序ランダム化のうえ {judge} がペア比較し、成果物未提出はルールにより敗北。レーティングはBradley-Terry適合で、XBoardのXDRatingと同一スケール。コストは実トークン使用量×定価で計測。審査モデル自身のエントリもプールで競いますが、審査員が自社モデルを含むペアを採点することはなく、そのペアは他の審査員が判定します。すべての比較は匿名化・順序ランダム化されています。ModelXD Router行はModelXD自身のサービスの実測値です。ライブラリ内の各タスクについて、そのタスクで最良と実測されたエントリを提供するため、その対戦とコストは提供された実行の実際の判定と実費です——完了した測定に基づく選択であり、他のエントリはすべてブラインドで実行されました。本ページのレーティングはGDPval-AAとは比較できません（審査・プール・アンカーが異なります）。', ko: '각 엔트리는 (모델 × 추론 강도) 조합으로, 오픈소스 Stirrup 하니스에서 {tasks}개의 GDPval 골드 과제를 실행합니다(Docker 샌드박스, 캐시된 웹 환경: 동일 쿼리 → 동일 결과). 과제는 GDPval 9개 섹터에 균형 배분됩니다. 엔트리마다 수행한 과제 수가 다를 수 있으며, 대국 수 열이 각 엔트리의 범위를 보여줍니다. 결과물은 익명화·순서 무작위화 후 {judge}가 쌍대 비교하며, 결과물 미제출 시 규칙상 패배 처리됩니다. 레이팅은 Bradley-Terry 적합으로 XBoard의 XDRating과 동일한 스케일입니다. 비용은 실제 토큰 사용량 × 정가로 측정됩니다. 심사 모델 자신의 엔트리도 풀에서 경쟁하지만, 어떤 심사도 자사 모델이 포함된 쌍은 평가하지 않으며 그 쌍은 다른 심사들이 판정합니다. 모든 비교는 익명화·순서 무작위화됩니다. ModelXD Router 행은 ModelXD 자체 서비스의 실측값입니다. 라이브러리의 각 과제에 대해 해당 과제에서 가장 좋게 측정된 엔트리를 제공하므로, 그 대국과 비용은 제공된 실행의 실제 판정과 실제 비용입니다 — 완료된 측정에 기반한 선택이며, 다른 모든 엔트리는 블라인드로 실행되었습니다. 이 페이지의 레이팅은 GDPval-AA와 직접 비교할 수 없습니다(심사·풀·앵커가 다릅니다).' },
