...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.tb.method.body': { en: 'Each entry is a (model, reasoning-effort) pair run on {n} {set} tasks through the open-source Harbor harness with the {harness} agent — real terminal environments in Docker. Scoring is by each task’s own verifier tests: binary pass/fail, no LLM judges. Tasks were drawn one eval + one test pair per category, difficulty-matched. Costs are measured from actual token usage at list prices (cached input at the cached rate). Cells are run strictly one at a time so no task is slowed by a neighbour.', 'zh-Hant': '每個條目為（模型 × 推理強度）組合，透過開源 Harbor 框架以 {harness} 代理執行 {n} 個 {set} 任務——Docker 中的真實終端環境。評分由各任務自帶的驗證測試判定：二元通過/失敗，無 LLM 評審。任務按每個類別各取 eval + test 一對、難度匹配。成本按實際 token 用量以標價計算（快取輸入按快取價）。所有單元嚴格逐一執行，任務不會被鄰居拖慢。', 'zh-Hans': '每个条目为（模型 × 推理强度）组合，通过开源 Harbor 框架以 {harness} 代理执行 {n} 个 {set} 任务——Docker 中的真实终端环境。评分由各任务自带的验证测试判定：二元通过/失败，无 LLM 评审。任务按每个类别各取 eval + test 一对、难度匹配。成本按实际 token 用量以标价计算（缓存输入按缓存价）。所有单元严格逐一执行，任务不会被邻居拖慢。', ja: '各エントリは（モデル × 推論エフォート）の組で、オープンソースのHarborハーネス上で{harness}エージェントにより{n}件の{set}タスクを実行——Docker内の実ターミナル環境。採点は各タスク自身の検証テストによる二値の合否で、LLM審査はありません。タスクはカテゴリごとにeval+testの難易度一致ペアを1組ずつ抽出。コストは実トークン使用量×定価（キャッシュ入力はキャッシュ価格）。セルは厳密に1件ずつ実行され、隣のタスクに遅くされることはありません。', ko: '각 엔트리는 (모델 × 추론 강도) 조합으로, 오픈소스 Harbor 하니스에서 {harness} 에이전트로 {n}개의 {set} 과제를 실행합니다 — Docker 내 실제 터미널 환경. 채점은 각 과제 자체 검증 테스트의 이진 통과/실패이며 LLM 심사는 없습니다. 과제는 카테고리별로 eval+test 난이도 일치 쌍을 1쌍씩 추출했습니다. 비용은 실제 토큰 사용량 × 정가(캐시 입력은 캐시 요금)로 측정됩니다. 셀은 엄격히 한 번에 하나씩 실행되어 이웃 과제에 의해 느려지지 않습니다.' },
