...
export const STRINGS: Record<string, Entry> = {
  ...
  'xeval.lead.tb': { en: 'ModelXD runs {set} in the open-source Harbor harness with the {harness} agent, scored by each task’s own verifier tests. {top} solves {solved} of {n} tasks at {cost} per task.', 'zh-Hant': 'ModelXD 以開源 Harbor 框架搭配 {harness} 代理執行 {set}，由各任務自帶的驗證測試計分。{top} 解出 {n} 題中的 {solved} 題，每題 {cost}。', 'zh-Hans': 'ModelXD 以开源 Harbor 框架搭配 {harness} 代理执行 {set}，由各任务自带的验证测试计分。{top} 解出 {n} 题中的 {solved} 题，每题 {cost}。', ja: 'ModelXDはオープンソースのHarborハーネスと{harness}エージェントで{set}を実行し、各タスク自身の検証テストで採点します。{top}は{n}問中{solved}問を解き、1問あたり{cost}です。', ko: 'ModelXD는 오픈소스 Harbor 하니스와 {harness} 에이전트로 {set}을 실행하며 각 과제 자체 검증 테스트로 채점합니다. {top}은 {n}개 중 {solved}개를 해결했고 과제당 {cost}입니다.' },
  'xeval.about':        { en: 'About this benchmark', 'zh-Hant': '關於此基準', 'zh-Hans': '关于此基准', ja: 'このベンチマークについて', ko: '이 벤치마크 소개' },
  'xeval.about.source': { en: 'Source', 'zh-Hant': '來源', 'zh-Hans': '来源', ja: 'ソース', ko: '출처' },
  'xeval.about.lab': { en: 'Harvey — the legal-AI company — open-sourced LAB to measure whether agents can do real legal work: 1,671 tasks across 24 practice areas, each one a realistic matter with its own client files. The agent gets a container, six file tools and no internet, then must deliver a work product a partner would review — an issues memorandum, a markup, a drafted agreement. Every task carries a rubric written by practising lawyers, and it counts only if every criterion passes, because a diligence memo that catches most of the issues and misses one material one is not partly right.', 'zh-Hant': 'Harvey（法律 AI 公司）將 LAB 開源，用以衡量 AI 代理能否完成真實法律工作：24 個專業領域、1,671 個任務，每題都是配有客戶檔案的真實案件。代理僅有容器、六項檔案工具、無網路，必須交出合夥人會審閱的工作成果——爭點備忘錄、修訂標記、協議草稿。每題皆附執業律師撰寫的評分標準，且必須全項通過才算通過：一份抓到多數問題卻漏掉一項關鍵問題的盡調備忘錄，並非「部分正確」。', 'zh-Hans': 'Harvey（法律 AI 公司）将 LAB 开源，用以衡量 AI 代理能否完成真实法律工作：24 个专业领域、1,671 个任务，每题都是配有客户档案的真实案件。代理仅有容器、六项文件工具、无网络，必须交出合伙人会审阅的工作成果——争点备忘录、修订标记、协议草稿。每题皆附执业律师撰写的评分标准，且必须全项通过才算通过：一份抓到多数问题却漏掉一项关键问题的尽调备忘录，并非「部分正确」。', ja: '法務AI企業のHarveyが、エージェントに実際の法務が務まるかを測るために公開したのがLABです。24の専門分野にわたる1,671タスクで、いずれも実案件のクライアント資料を伴います。エージェントはコンテナと6つのファイルツールのみ、インターネットなしで、パートナーが査読する成果物——論点メモ、マークアップ、契約ドラフト——を提出しなければなりません。各タスクには実務弁護士が書いたルーブリックが付き、全項目を満たした場合のみ合格です。重要な論点を一つ落としたデューデリメモは「部分的に正しい」とは言えないからです。', ko: '법률 AI 기업 Harvey가 에이전트가 실제 법률 업무를 해낼 수 있는지 측정하기 위해 공개한 것이 LAB입니다. 24개 실무 분야에 걸친 1,671개 과제이며, 각각 실제 사건의 고객 파일을 포함합니다. 에이전트는 컨테이너와 6개 파일 도구만으로, 인터넷 없이 파트너가 검토할 결과물 — 쟁점 메모, 마크업, 계약 초안 — 을 제출해야 합니다. 각 과제에는 현업 변호사가 작성한 루브릭이 있으며 모든 기준을 충족해야만 통과입니다. 핵심 쟁점 하나를 놓친 실사 메모는 «부분적으로 맞다»고 할 수 없기 때문입니다.' },
  'xeval.about.tb': { en: 'Terminal-Bench measures agents in real command-line environments: each task is a containerised machine with a job to finish — build a library from source, recover a corrupted database, break a filter, train a model — and the task’s own test suite decides pass or fail. No judges, no partial credit. Version 2.1 is the verified iteration of 2.0, with 26 tasks fixed for bugs, timeouts and reward-hacking.', 'zh-Hant': 'Terminal-Bench 在真實命令列環境中衡量代理能力：每題是一台容器化機器與一項待完成工作——從原始碼建置函式庫、修復損毀資料庫、突破過濾器、訓練模型——由該題自帶的測試套件判定通過與否。無評審、無部分分數。2.1 版是 2.0 的驗證版，修正了 26 題的錯誤、逾時與獎勵破解問題。', 'zh-Hans': 'Terminal-Bench 在真实命令行环境中衡量代理能力：每题是一台容器化机器与一项待完成工作——从源码构建库、修复损坏数据库、突破过滤器、训练模型——由该题自带的测试套件判定通过与否。无评审、无部分分数。2.1 版是 2.0 的验证版，修正了 26 题的错误、超时与奖励破解问题。', ja: 'Terminal-Benchは実際のコマンドライン環境でエージェントを測ります。各タスクはコンテナ化されたマシンと完遂すべき仕事——ソースからのライブラリ構築、破損データベースの復旧、フィルタの突破、モデルの学習——で、合否はタスク自身のテストスイートが判定します。審査員なし、部分点なし。2.1は2.0の検証版で、バグ・タイムアウト・報酬ハッキングについて26タスクが修正されています。', ko: 'Terminal-Bench는 실제 커맨드라인 환경에서 에이전트를 측정합니다. 각 과제는 컨테이너화된 머신과 완수해야 할 작업 — 소스에서 라이브러리 빌드, 손상된 데이터베이스 복구, 필터 우회, 모델 학습 — 이며 합격 여부는 과제 자체의 테스트 스위트가 결정합니다. 심사도 부분 점수도 없습니다. 2.1은 2.0의 검증판으로 버그·타임아웃·보상 해킹에 대해 26개 과제가 수정되었습니다.' },
  'xeval.lead.lab': { en: 'We run {n} of these tasks in Harvey’s own harness and grade every rubric criterion with two independent judges. A task counts only when EVERY criterion passes, which is why these numbers sit far below per-criterion accuracy: {best} satisfies {bestcov} of all criteria yet fully passes just {bestsolved} of {n} tasks. Serving each practice area’s measured best model, {top} fully passes {solved}.', 'zh-Hant': '我們在 Harvey 官方框架中執行 {n} 個任務，並由兩位獨立評審逐項評分。必須「全部」項目通過才算通過，因此這些數字遠低於單項通過率：{best} 滿足全部項目的 {bestcov}，卻僅在 {n} 題中完全通過 {bestsolved} 題。依各專業領域實測最佳模型提供服務後，{top} 完全通過 {solved} 題。', 'zh-Hans': '我们在 Harvey 官方框架中执行 {n} 个任务，并由两位独立评审逐项评分。必须「全部」项目通过才算通过，因此这些数字远低于单项通过率：{best} 满足全部项目的 {bestcov}，却仅在 {n} 题中完全通过 {bestsolved} 题。依各专业领域实测最佳模型提供服务后，{top} 完全通过 {solved} 题。', ja: 'Harvey公式のハーネスで{n}件を実行し、各ルーブリック項目を2名の独立審査が採点します。全項目を満たした場合のみ合格となるため、項目単位の正答率よりはるかに低い数字になります：{best}は全項目の{bestcov}を満たしながら、完全通過は{n}件中{bestsolved}件です。各分野で実測最良のモデルを提供すると、{top}は{solved}件を完全通過します。', ko: 'Harvey 공식 하니스에서 {n}개 과제를 수행하고, 각 루브릭 기준을 두 독립 심사가 채점합니다. 모든 기준을 충족해야만 통과이므로 기준별 정답률보다 훨씬 낮은 수치가 나옵니다: {best}는 전체 기준의 {bestcov}를 충족하지만 완전 통과는 {n}개 중 {bestsolved}개입니다. 각 실무 분야의 실측 최고 모델을 제공하면 {top}은 {solved}개를 완전 통과합니다.' },
  ...
  'xtell.step.birth':  { en: '1 · Your birth moment', 'zh-Hant': '一 · 稟明生辰', 'zh-Hans': '一 · 禀明生辰', ja: '一 · 生年月日時', ko: '일 · 생년월일시' },
  'xtell.step.chart':  { en: '2 · The chart — verify it', 'zh-Hant': '二 · 排盤（請核對）', 'zh-Hans': '二 · 排盘（请核对）', ja: '二 · 命盤（ご確認を）', ko: '이 · 명반 (확인하세요)' },
  'xtell.step.master': { en: '3 · Ask the master', 'zh-Hant': '三 · 請老師', 'zh-Hans': '三 · 请老师', ja: '三 · 老師に聞く', ko: '삼 · 스승에게 묻기' },
  ...
  'xtell.compute':     { en: 'Cast the chart', 'zh-Hant': '排盤', 'zh-Hans': '排盘', ja: '命盤を立てる', ko: '명반 세우기' },
  ...
  'xtell.chart.note':  { en: 'Computed by an open-source calendar engine (solar-term exact). Check it against any chart site — then ask the master.', 'zh-Hant': '由開源曆法引擎計算（節氣精確）。歡迎拿去任何排盤網站核對，再請老師解讀。', 'zh-Hans': '由开源历法引擎计算（节气精确）。欢迎拿去任何排盘网站核对，再请老师解读。', ja: 'オープンソースの暦エンジンで計算（節気に正確）。他の命盤サイトと照合してから、老師にお尋ねください。', ko: '오픈소스 역법 엔진으로 계산(절기 정확). 다른 명반 사이트와 대조한 뒤 스승에게 물어보세요.' },
  'xtell.pickmodel':   { en: 'Choose a master (model)', 'zh-Hant': '選一位老師（模型）', 'zh-Hans': '选一位老师（模型）', ja: '老師（モデル）を選ぶ', ko: '스승(모델) 선택' },
  'xtell.search':      { en: 'Allow web search', 'zh-Hant': '允許查網路', 'zh-Hans': '允许查网络', ja: 'ウェブ検索を許可', ko: '웹 검색 허용' },
  'xtell.consult':     { en: 'Ask for the reading', 'zh-Hant': '請老師批命', 'zh-Hans': '请老师批命', ja: '鑑定を頼む', ko: '풀이 요청' },
  'xtell.consulting':  { en: 'Reading…', 'zh-Hant': '批命中…', 'zh-Hans': '批命中…', ja: '鑑定中…', ko: '풀이 중…' },
