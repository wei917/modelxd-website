...
-- =============================================================
-- Creates table (private studio generations)
-- =============================================================
...
create policy "creates: owner read"
  on creates for select using (auth.uid() = user_id);

create policy "creates: owner insert"
  on creates for insert with check (auth.uid() = user_id);

create policy "creates: owner delete"
  on creates for delete using (auth.uid() = user_id);
...
create policy "owner access" on attachments for all using (auth.uid() = user_id);

-- Add attachment_id to duels and creates
