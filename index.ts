...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "PATCH") {
      ...
      const body = await req.json();
      const { id, hero_image, journey_map_image, day_images, ai_video_job_id, ai_video_status, ai_video_url, itinerary, destination, hero_image_status, journey_map_status } = body;
      ...
      if (hero_image_status !== undefined) updates.hero_image_status = hero_image_status;
      if (journey_map_status !== undefined) updates.journey_map_status = journey_map_status;
