...
const vert = renderPlan({ ...emptyTimeline('9:16'), settings: { resolution: '720p', muteClips: true, dissolve: 0, burnSubtitles: true } })
check('burnSubtitles off empties the plan subtitles', renderPlan({ ...tl, settings: { ...tl.settings, burnSubtitles: false } }).subtitles.length === 0 && renderPlan(tl).subtitles.length === 2)
