...
|---|---|---|---|
| 2026-09-27 | alibaba | Claude | **Qwen3 TTS added + enabled** — `qwen3-tts-instruct-flash`, $0.115/10k = **$11.50 per 1M input characters**, output free. ~5x cheaper than MiniMax Turbo on the same line (繁體 $0.0007 vs $0.0037). Alibaba was missed in the Sep 25 TTS survey (owner caught it): the shortlist came from TTS names I knew, not from the providers we already call. Their meter counts each CJK character as **2** (a 24-char 繁體 line billed 61) and we bill `usage.characters`, their number. Audio arrives as an EXPIRING http URL, downloaded immediately, never stored. Four listed voices (Jennifer/Ryan/Katerina/Aiden) are `qwen3-tts-flash` only and 400 on the instruct row — dropped, 10 remain. Not taken: CosyVoice v3 ($13-26/1M) is the cloning/voice-design line and WebSocket-only; qwen-audio-3.0-tts (Alibaba's new recommendation) is also WebSocket-only. |
| 2026-09-23 | openai, anthropic, xai, google, alibaba | Claude | **Six models added + enabled** after a live smoke test each: gpt-6-sol (2/0.2/10), gpt-6-luna (0.1/0.01/0.5), claude-opus-5-5 (4/0.2/20, search 0.01), grok-4.7 (2/0.5/6), gemini-3.8-flash (1.5/0.15/7.5, search 0.014), qwen3.8-flash (0.15/0.015/0.47, search 0.01). Effort sets probed on the wire: GPT-6 Sol/Luna none→max, Grok 4.7 low→xhigh (no none/max), Gemini 3.8 Flash low/medium/high (minimal errors), Qwen binary. Promos NOT carried: Gemini 3.8 Flash 0.75/3.75/0.075 to Dec 31; gpt-5.6-sol still promo-priced on the page. **Unverified:** qwen3.8-flash cached_input — Alibaba publishes none per model; carried at the page-wide 10% explicit-hit rule (0.015), the low end, so a cache hit can never bill above their charge. **Not added:** claude-mythos-5/5.1 (invite-only, Project Glasswing — our key cannot call them); MiniMax-M3 (needs a text path in code, and its 'permanent 50% off' is ambiguous as list vs promo — owner call). **Known under-bill, unfixed:** OpenAI charges cache WRITES at 1.25x input on gpt-6-astra and the 5.6 family (automatic, no opt-out) and we model no cache-write term; OpenAI reports no cache-write token count, so it is not reliably derivable. Anthropic's writes are already folded at 1.25x. xAI web search ($5/1k) stays uncharged because our xai.ts never requests search. |
| 2026-09-22 | alibaba | Claude | **HappyHorse 1.1 480P tier added** to t2v/i2v/r2v: `per_video_second.480p = 0.07` (was missing, so a 480p pick fell back to 720p and billed $0.14/s — double). 480p also added to `output_config.video.sizes` + `durations_by_resolution` so the picker can offer it. Verified live: a 3s 480p t2v run rendered and billed $0.21 = $0.07/s exactly. Also confirmed on the pricing page: **no HappyHorse 1.2 exists** — 1.0 and 1.1 only. |
| 2026-09-15 | openai | Claude | **gpt-image-2.5-sunburst + gpt-image-2.5-flare added + enabled** (snapshots 2026-09-08). Same token rates as gpt-image-2: text 5 (cached 1.25), image in 8 (cached 2), image out 30. Billing is token-exact from usage; `per_image` is the ESTIMATE map, measured at 1024x1024 (low 0.006, medium 0.0133, high 0.0528, xhigh 0.0938, max 0.2109 — both models identical token counts) and scaled to other sizes by gpt-image-2's size ratios. Qualities low/medium/high/xhigh/max all accepted live (the API's enum error message still lists only low/medium/high/auto). Masked edit works on both. ~$0.80 of tests. |
| 2026-09-14 | all providers | Claude (subagent, read-only) | 52 enabled rows across 8 providers checked. **Fixed wan2.1-vace-plus** (never audited): `{480P: 0.015, 720P: 0.036}` → `{720p: 0.10}` — official list is 720P $0.10/s only. Worse than it looked: the uppercase keys never matched the lowercase lookup in pricing.ts, so every VACE run fell through to the FIRST key and billed the fictional 480P $0.015/s (~15% of list). **Unverified, left as is:** qwen3.8-max cached_input 0.25 (explicit cache-hit price is console-only; implicit cache ≈20% = 0.40). Promos seen, not applied: gpt-5.6-sol, Gemini 3.6/3.7/3.8 Flash (to Dec 31), wan3.0-video 30% off, happyhorse-1.0-video-edit 20% off; the HappyHorse 1.0/1.1 t2v/i2v/r2v promos have ended. Unmodeled: OpenAI cache-write rates, xAI web search $5/1K on grok-4.6, gpt-image-2 cached image input $2. Other 50 rows correct. |
...
| 2026-08-26 | alibaba (wan3.0 addition) | Claude | **Added `wan3.0-video`** (invitational preview — our key has access; probed live): unified any-to-video, list $0.05/$0.10/$0.20 per s (480P/720P/1080P), durations 2–30s (`-1` = auto), 30fps, input+output duration billed (handled — usage.duration drives billing, verified $0.808 = 3.08s in + 5s out). Inputs: text, first/last frame, reference_image/video/audio. **Reference audio: wav/mp3 only, ≤15s** (m4a and 18s both rejected upstream). All six input modes smoked live (~$4). Fixed two 480P mapping gaps in alibaba.ts (request coerced 480p→720P; billing keyed SR 480→'720p' — a silent 2× overcharge, caught before enabling). |
| 2026-08-26 | google (YouTube URL video input) | Claude | **No change — deliberately.** The new XDirect reference-video path sends a YouTube URL to `gemini-3.1-flash-lite` (analysis) and `gemini-3.1-flash-image` (style frames). usageMetadata reports real VIDEO prompt tokens (22,752 and 65,750 on one live run), which reads like a costing gap on the image rows — they carry no `video_input` rate, so the video term resolves to $0. **It is not a gap: Google's YouTube-URL input is in PREVIEW and billed at no charge.** Do not copy the model's text/image rate onto `video_input` to "fix" it — that over-reports a free feature. Google states preview pricing and rate limits will change; when a real rate is published, add `video_input` to the image rows and `calcTextCost` picks it up with no code change. flash-lite already carries video_input 0.25 (correct for its own non-preview video input). Source: ai.google.dev/gemini-api/docs/pricing + /video-understanding. |
| 2026-09-02 | ALL text providers (thinking levels) | Claude | **Audited declared `thinking_levels` against what each API actually accepts** — probe an invalid level, the error enumerates the real set (rejections are free). One gap, now fixed: **every OpenAI row was missing `max`**, so the top reasoning tier was unreachable in XCreate (added to luna/terra/sol/5.5; `minimal` is correctly excluded — all five models reject it). Verified correct, no change: anthropic ×5 = low→max (Models API `capabilities.effort`), kimi-k3 = low/high/max (`medium` genuinely rejected), gemini-3.6-flash = minimal/low/medium/high (`MAX` rejected), grok-4.6 = low/medium/high/xhigh (`none`+`max` rejected), qwen = binary thinking_true/false. Measured while there (gpt-5.6-luna, same prompt): effort buys tokens, not slower tokens — 71-89 tok/s flat across the ladder while output grows 693→1,759 and reasoning 0→1,034. |
| 2026-08-29 | anthropic (Fable 5.1) | Claude | **claude-fable-5-1 added + enabled.** Models API: 1M in / 128K out, adaptive-only thinking, effort low→max, image+PDF input. Pricing page: **10 / 50, cache read $0.25** — 5.1 and Mythos 5.1 use a **0.025x** cache multiplier instead of the usual 0.1x (so 4x cheaper cached input than Fable 5's $1), no promo caveat, permanent list. Smoked live: effort low + max, and web search (1 search counted, cost exact to the cent). No code change — anthropic.ts already maps our thinking level to output_config.effort with adaptive thinking. |
| 2026-08-27 | google (omni 1.1) | Claude | **All 4 declared modes verified live** on 1.1 — text_to_video, image_to_video, reference_to_video (2 refs), edit (Files-API upload of the source clip): every one billed exactly $0.30 for 3s (17,376 video tokens = 3.0s at 5,792/s). Free enum probe (bogus task → API lists valid ones) shows a 5th task we do NOT expose: **`extend`** — same family as the Veo extension, unwired. Known gap: omni INPUT tokens are unbilled (an edit consumes ~17k video-input tokens ≈ $0.026 absorbed per 3s edit) — we under-bill, never over. |
| 2026-08-27 | google (omni 1.1) | Claude | **gemini-omni-1.1-flash added + enabled** — GA successor to gemini-omni-flash-preview, same published price ($1.50 in; $17.50/1M video out = 5,792 tok/s of 720p ≈ $0.10/s) and same generateContent interface, so no code change (the router keys on the `gemini-omni` prefix). Smoked live: 3s/720p h264+aac, 17,376 video tokens = exactly 3.0s, billed $0.30. Preview row left enabled — no deprecation notice published. |
